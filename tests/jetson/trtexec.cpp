...
using namespace infer::trt::sample;
...
bool printLayerInfo(const ReportingOptions &reporting,
  ...
  if (reporting.layerInfo) {
    ...
    sample::gLogInfo << getLayerInformation(
                            iEnv, nvinfer1::LayerInformationFormat::kONELINE)
                     << std::flush;
  }
  if (!reporting.exportLayerInfo.empty()) {
    std::ofstream os(reporting.exportLayerInfo, std::ofstream::trunc);
    os << getLayerInformation(iEnv, nvinfer1::LayerInformationFormat::kJSON)
       << std::flush;
  }
  return true;
}

void printPerformanceProfile(const ReportingOptions &reporting,
                             const InferenceEnvironment &iEnv) {
...
void printOutput(const ReportingOptions &reporting,
                 const InferenceEnvironment &iEnv, int32_t batch) {
  if (reporting.output) {
    dumpOutputs(*iEnv.context.front(), *iEnv.bindings.front(),
                sample::gLogInfo);
  }
  if (!reporting.exportOutput.empty()) {
    exportJSONOutput(*iEnv.context.front(), *iEnv.bindings.front(),
                     reporting.exportOutput, batch);
  }
}
...
int main(int argc, char **argv) {
    ...
    try {
      ...
      if (!args.empty()) {
        for (const auto &arg : args) {
          sample::gLogError << "Unknown option: " << arg.first << " "
                            << arg.second << std::endl;
        }
        failed = true;
      }
  ...
  sample::gLogInfo << "TensorRT version: " << NV_TENSORRT_MAJOR << "."
                   << NV_TENSORRT_MINOR << "." << NV_TENSORRT_PATCH
                   << std::endl;
  initLibNvInferPlugins(&sample::gLogger.getTRTLogger(), "");

  for (const auto &pluginPath : options.system.plugins) {
    sample::gLogInfo << "Loading supplied plugin library: " << pluginPath
                     << std::endl;
    loadLibrary(pluginPath);
  }

  if (options.build.safe && !sample::hasSafeRuntime()) {
    sample::gLogError << "Safety is not supported because safety runtime "
  ...
  if (!options.build.safe && options.build.consistency) {
    sample::gLogInfo << "Skipping consistency checker on non-safety mode."
                     << std::endl;
    options.build.consistency = false;
  }

  InferenceEnvironment iEnv;
  UniquePtr<INetworkDefinition> networkForRefit;
  Parser parserHoldingWeightsMem;
  {
    // Scope the build phase so any held memory is released before moving to
    // inference phase.
    BuildEnvironment bEnv;
    const time_point buildStartTime{std::chrono::high_resolution_clock::now()};
    bool buildPass = getEngineBuildEnv(options.model, options.build,
                                       options.system, bEnv, sample::gLogError);
    ...
    sample::gLogInfo << "Engine " << (options.build.load ? "loaded" : "built")
                     << " in "
                     << duration(buildEndTime - buildStartTime).count()
                     << " sec." << std::endl;
  ...
  if (!options.build.safe && iEnv.engine.get()->isRefittable()) {
    ...
    if (options.inference.timeRefit) {
      if (networkForRefit.operator bool()) {
        ...
        sample::gLogWarning << "Network not available, skipped timing refit."
                            << std::endl;
      }
    }
  }
  ...
  if (options.inference.timeDeserialize) {
    ...
    // Release the serialized memory when not in use before allocating bindings
  ...
  if (options.build.safe && options.system.DLACore >= 0) {
    sample::gLogInfo << "Safe DLA capability is detected. Please save DLA "
                        "loadable with --saveEngine option, "
                        "then use dla_safety_runtime to run inference with "
                        "saved DLA loadable, "
  ...
  const bool profilerEnabled =
      options.reporting.profile || !options.reporting.exportProfile.empty();
  if (profilerEnabled && !options.inference.rerun) {
    iEnv.profiler.reset(new Profiler);
    if (options.inference.graph &&
        (getCudaDriverVersion() < 11010 || getCudaRuntimeVersion() < 11000)) {
      options.inference.graph = false;
      sample::gLogWarning << "Graph profiling only works with CUDA 11.1 and "
                             "beyond. Ignored --useCudaGraph flag "
  ...
  if (profilerEnabled && !options.inference.rerun) {
    sample::gLogWarning << "The network timing report will not be accurate due "
                           "to extra synchronizations "
                           "when profiler is enabled."
                        << std::endl;
    sample::gLogWarning
        << "Add --separateProfileRun to profile layer timing in a separate run."
        << std::endl;
  }
  ...
  printPerformanceReport(trace, options.reporting,
                         ...
                         sample::gLogWarning, sample::gLogVerbose);
