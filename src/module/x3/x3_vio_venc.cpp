...
int x3_venc_h264avbr(VENC_RC_ATTR_S *pstRcParam, VENC_H264_AVBR_S *pstH264AVbr);
...
int x3_venc_h265avbr(VENC_RC_ATTR_S *pstRcParam, VENC_H265_AVBR_S *pstH265AVbr);
...
int x3_venc_setgop(VENC_GOP_ATTR_S *pstGopAttr, int presetidx, int refreshtype,
                   int qp, int intraqp);
...
int VencChnAttrInit(VENC_CHN_ATTR_S *pVencChnAttr, PAYLOAD_TYPE_E p_enType,
  ...
  if (p_enType == PT_H264) {
    ...
    pVencChnAttr->stVencAttr.stAttrH264.h264_profile =
        VENC_H264_PROFILE_E::HB_H264_PROFILE_UNSPECIFIED;
    pVencChnAttr->stVencAttr.stAttrH264.h264_level =
        HB_H264_LEVEL_E::HB_H264_LEVEL_UNSPECIFIED;
  }
...
void venc_thread(void *vencpram) { printf("venc chn xxx thread exit\n"); }
...
int x3_venc_initattr(int VeChn, VENC_CHN_ATTR_S *vencChnAttr) {
  ...
  if (ptype == PT_H264) {
    ...
    case VENC_RC_MODE_H264QPMAP: {
      ...
      stH264QpMap.u32QpMapArray = reinterpret_cast<unsigned char *>(
          malloc(stH264QpMap.u32QpMapArrayCount));
      for (size_t i = 0; i < stH264QpMap.u32QpMapArrayCount; i++) {
    ...
    case VENC_RC_MODE_H265QPMAP: {
      ...
      stH265QpMap.u32QpMapArrayCount =
          ((vencChnAttr->stVencAttr.u32PicWidth + 0x0f) & ~0x0f) / 16 *
          ((vencChnAttr->stVencAttr.u32PicHeight + 0x0f) & ~0x0f) / 16;
      stH265QpMap.u32QpMapArray = reinterpret_cast<unsigned char *>(
          malloc(stH265QpMap.u32QpMapArrayCount));
...
int x3_venc_h264qpmap(VENC_RC_ATTR_S *pstRcParam,
  ...
  pstRcParam->stH264QpMap.u32QpMapArrayCount = pstH264QpMap->u32QpMapArrayCount;
...
int x3_venc_h265qpmap(VENC_RC_ATTR_S *pstRcParam,
  ...
  pstRcParam->stH264QpMap.u32QpMapArrayCount = pstH265QpMap->u32QpMapArrayCount;
  pstRcParam->stH264QpMap.u32QpMapArray = pstH265QpMap->u32QpMapArray;
...
int x3_venc_mjpgfixqp(VENC_RC_ATTR_S *pstRcParam,
  ...
  pstRcParam->stMjpegFixQp.u32QualityFactort = pstMjpegFixQp->u32QualityFactort;
 ...
 ******************************************************************************/
uint32_t x3_venc_get_file_postfix(PAYLOAD_TYPE_E enPayload,
                                  char *szFilePostfix) {
 ...
 ******************************************************************************/
uint32_t x3_venc_save_stream(PAYLOAD_TYPE_E enType, FILE *pFd,
