...
//! argument and return true. If it does not: return false.
template <typename T>
...
void processShapes(std::unordered_map<std::string, ShapeRange> &shapes,
      ...
      || (!minShapes && maxShapes && optShapes)) // both maxShapes and optShapes
  {
    if (calib) {
      ...
      throw std::invalid_argument("Must specify only --optShapes or all of "
...
void BuildOptions::parse(Arguments &arguments) {
  ...
  if (addedExplicitBatchFlag) {
    ...
    FLOWENGINE_LOGGER_WARN(
        "Explicit batch dim is automatically enabled if input model is ONNX or "
        "if dynamic shapes are provided when the engine is built.");
  }
  ...
  if (profilingVerbosityString == "layer_names_only") {
    ...
    FLOWENGINE_LOGGER_WARN("--profilingVerbosity=default has been deprecated "
                           "by --profilingVerbosity=layer_names_only.");
    profilingVerbosity = nvinfer1::ProfilingVerbosity::kLAYER_NAMES_ONLY;
  } else if (profilingVerbosityString == "verbose") {
    FLOWENGINE_LOGGER_WARN("--profilingVerbosity=verbose has been deprecated "
  ...
  if (!helps) {
    ...
    if (build.safe && system.DLACore >= 0) {
      ...
      if (!checkSafeDLAFormats(build.inputFormats) ||
        ...
        throw std::invalid_argument("I/O formats for safe DLA capability are "
                                    "restricted to fp16:chw16 or int8:chw32");
      }
      if (system.fallback) {
        throw std::invalid_argument("GPU fallback (--allowGPUFallback) not "
                                    "allowed for safe DLA capability");
      }
    }
  }
}
...
std::ostream &operator<<(std::ostream &os, const ModelOptions &options) {
  ...
  case ModelFormat::kONNX: // Fallthrough: No options to report for ONNX or the
                           // generic case
  case ModelFormat::kANY:
    break;
  }
