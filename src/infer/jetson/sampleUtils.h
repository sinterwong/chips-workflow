...
inline nvinfer1::Dims toDims(const std::vector<int> &vec) {
  ...
  if (static_cast<int>(vec.size()) > limit) {
    FLOWENGINE_LOGGER_WARN("Vector too long, only first 8 elements are used in dimension.");
  }
...
struct Binding {
  ...
  void fill(const std::string &fileName) {
    ...
    if (file.is_open()) {
      file.read(static_cast<char *>(buffer->getHostBuffer()),
                buffer->getSize());
      file.close();
    } else {
...
inline std::vector<char> loadTimingCacheFile(const std::string inFileName) {
  ...
  if (!iFile) {
    FLOWENGINE_LOGGER_WARN("Could not read timing cache from: {}. A new timing cache will be generated and written.", inFileName);
  ...
  iFile.close();
  FLOWENGINE_LOGGER_INFO("Loaded {} bytes of timing cache from {}", fsize, inFileName);
  return content;
}
...
inline void saveTimingCacheFile(const std::string outFileName,
  ...
  if (!oFile) {
    FLOWENGINE_LOGGER_WARN("Could not write timing cache to: {}", outFileName);
    return;
  }
  oFile.write((char *)blob->data(), blob->size());
  oFile.close();
  FLOWENGINE_LOGGER_INFO("Saved {} bytes of timing cache to {}", blob->size(), outFileName);
}
