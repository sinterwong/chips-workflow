...
private:
  ...
  void createEnqueueFunction(const InferenceOptions &inference,
    ...
    if (inference.graph) {
      ...
      // at least once before starting CUDA graph capture.
      ...
      // under CUDA graph capture mode. When the stream is capturing, the
      // enqueue call may return false if the current CUDA graph capture fails.
      if (mEnqueue(stream)) {
...
bool timeDeserialize(InferenceEnvironment &iEnv) {
  ...
  auto timeDeserializeFn = [&]() -> float {
    ...
    if (iEnv.safe) {
      safeEngine.reset(safeRT->deserializeCudaEngine(serializedEngine->data(),
                                                     serializedEngine->size()));
      deserializeOK = (safeEngine != nullptr);
    } else {
      engine.reset(rt->deserializeCudaEngine(
  ...
  // susceptible to caching issues causing problems in the first timing.
