...
struct Parser {
  UniquePtr<nvcaffeparser1::ICaffeParser> caffeParser;
  UniquePtr<nvuffparser::IUffParser> uffParser;
  UniquePtr<nvonnxparser::IParser> onnxParser;
...
struct BuildEnvironment {
  ...
  //! when ~BuildEnvironment() executes, the parser is destroyed before the
...
//! corresponding network definition, because its correctness may rely on the
//! constants.
//!
...
//! \return Pointer to the engine created or nullptr if the creation failed
//!
inline UniquePtr<nvinfer1::ICudaEngine>
getEngine(const ModelOptions &model, const BuildOptions &build,
...
bool timeRefit(const INetworkDefinition &network,
