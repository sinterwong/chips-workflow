...
#define CHECK(status)                                                          \
  do {                                                                         \
    auto ret = (status);                                                       \
    if (ret != 0) {                                                            \
      FLOWENGINE_LOGGER_ERROR("Cuda failure: {}", ret);                        \
      abort();                                                                 \
    }                                                                          \
  } while (0)
...
#define CHECK_RETURN_W_MSG(status, val, errMsg)                                \
  do {                                                                         \
    if (!(status)) {                                                           \
      FLOWENGINE_LOGGER_ERROR("{} Error in {}, function {}(), line {}",        \
                              errMsg, __FILE__, FN_NAME, __LINE__);            \
      return val;                                                              \
    }                                                                          \
  } while (0)
...
#define ASSERT(condition)                                                      \
  do {                                                                         \
    if (!(condition)) {                                                        \
      FLOWENGINE_LOGGER_ERROR("Assertion failure: {}", #condition);            \
      abort();                                                                 \
    }                                                                          \
  } while (0)
...
template <typename T> std::shared_ptr<T> infer_object(T *obj) {
  if (!obj) {
...
std::vector<std::string> classify(const std::vector<std::string> &refVector,
                                  ...
                                  const size_t topK) {
  const auto inds = argMagnitudeSort(output.cbegin(), output.cend());
...
std::vector<size_t> topKMagnitudes(const std::vector<T> &v, const size_t k) {
  std::vector<size_t> indices = argMagnitudeSort(v.cbegin(), v.cend());
  indices.resize(k);
  return indices;
}
...
inline void setDummyInt8DynamicRanges(const IBuilderConfig *c,
  ...
  if (c->getFlag(BuilderFlag::kINT8)) {
    FLOWENGINE_LOGGER_WARN(
        "Int8 calibrator not provided. Generating dummy per-tensor dynamic "
...
void readPPMFile(const std::string &filename, PPM<C, H, W> &ppm) {
...
inline void loadLibrary(const std::string &path) {
  ...
  // doing so. In order to get proper stack traces, don't delete the shared
  // library on close so that asan can resolve the symbols correctly.
  flags |= RTLD_NODELETE;
#endif // ENABLE_ASAN
  ...
  if (handle == nullptr) {
#ifdef _MSC_VER
    FLOWENGINE_LOGGER_ERROR("Could not load plugin library: {}", path);
#else
    FLOWENGINE_LOGGER_ERROR("Could not load plugin library: {}, due to: {}",
                            path, dlerror());
#endif
  }
}
...
inline bool isDataTypeSupported(DataType dataType) {
  auto builder = UniquePtr<nvinfer1::IBuilder>(
      nvinfer1::createInferBuilder(infer::trt::sample::gLogger.getTRTLogger()));
  if (!builder) {
    return false;
  }
