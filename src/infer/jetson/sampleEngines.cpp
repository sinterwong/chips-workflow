...
readScalesFromCalibrationCache(const std::string &calibrationFile) {
  ...
  if (!cache.is_open()) {
    FLOWENGINE_LOGGER_ERROR("[TRT] Can not open provided calibration cache file");
    return tensorScales;
  }
  std::string line;
...
Parser modelToNetwork(const ModelOptions &model,
                      ...
                      std::ostream &err) {
  FLOWENGINE_LOGGER_INFO("Start parsing network model");
  Parser parser;
  ...
  case ModelFormat::kONNX: {
    ...
    parser.onnxParser.reset(
        createParser(network, infer::trt::sample::gLogger.getTRTLogger()));
    if (!parser.onnxParser->parseFromFile(
            model.baseModel.model.c_str(),
            static_cast<int>(infer::trt::sample::gLogger.getReportableSeverity()))) {
  ...
  FLOWENGINE_LOGGER_INFO("Finish parsing network model");
  return parser;
}
...
bool setupNetworkAndConfig(const BuildOptions &build, const SystemOptions &sys,
  ...
  if (profile) {
    ...
    for (const auto &shape : build.shapes) {
      ...
      if (!tensorNameFound) {
        FLOWENGINE_LOGGER_ERROR("Cannot find input tensor with name {} in the network inputs! Please make sure the input tensor names are correct.", shape.first);
        return false;
      }
    }
  }
    ...
    if (profile) {
      ...
      if (isDynamicInput) {
        ...
        if (shape == build.shapes.end()) {
          ...
          // TODO
          // FLOWENGINE_LOGGER_WARN("Dynamic dimensions required for input: {}, but no shapes were provided. Automatically overriding shape to: {}", input->getName(), staticDims);
  ...
  if (!hasDynamicShapes && !build.shapes.empty()) {
    FLOWENGINE_LOGGER_ERROR("Static model does not take explicit shapes since the "
  ...
  if (build.int8 && !build.fp16) {
    FLOWENGINE_LOGGER_INFO("FP32 and INT8 precisions have been specified - more "
  ...
  if (!hasQDQLayers(network) && (build.int8 || int8IO) &&
    ...
    if (!hasQDQLayers(network) && int8IO) {
      try {
        ...
        FLOWENGINE_LOGGER_ERROR("Int8IO was specified but impossible to read tensor scales from provided calibration cache file");
        return false;
      }
    }
...
bool networkToEngine(const BuildOptions &build, const SystemOptions &sys,
                     ...
                     std::ostream &err) {
  UniquePtr<IBuilderConfig> config{builder.createBuilderConfig()};
  std::vector<std::vector<char>> sparseWeights;
  ...
  if (build.safe) {
    ...
    env.safeEngine.reset(safeRuntime->deserializeCudaEngine(
        env.serializedEngine->data(), env.serializedEngine->size()));
    if (build.consistency) {
      checkSafeEngine(env.serializedEngine->data(),
                      env.serializedEngine->size());
    }
    ...
    env.engine.reset(runtime->deserializeCudaEngine(
        env.serializedEngine->data(), env.serializedEngine->size()));
...
bool modelToBuildEnv(const ModelOptions &model, const BuildOptions &build,
                     ...
                     std::ostream &err) {
  UniquePtr<IBuilder> builder{
      createInferBuilder(infer::trt::sample::gLogger.getTRTLogger())};
...
bool loadEngineToEnv(const std::string &engine, int DLACore, bool safe,
  ...
  if (safe) {
    ...
    env.safeEngine.reset(
        safeRuntime->deserializeCudaEngine(engineData.data(), fsize));
...
void dumpRefittable(nvinfer1::ICudaEngine &engine) {
  UniquePtr<IRefitter> refitter{
      createInferRefitter(engine, infer::trt::sample::gLogger.getTRTLogger())};
  if (refitter == nullptr) {
    FLOWENGINE_LOGGER_ERROR("Failed to create a refitter.");
    return;
  }
  ...
  for (size_t i = 0; i < nbAll; ++i) {
    // TODO
    // FLOWENGINE_LOGGER_INFO("{} {}", layerNames[i], weightsRoles[i]);
...
bool saveEngine(const ICudaEngine &engine, const std::string &fileName,
  ...
  UniquePtr<IHostMemory> serializedEngine{engine.serialize()};
...
bool getEngineBuildEnv(const ModelOptions &model, const BuildOptions &build,
  ...
  if (build.save) {
    ...
    engineFile.write(static_cast<char *>(env.serializedEngine->data()),
                     env.serializedEngine->size());
...
bool serializeAndSave(const ModelOptions &model, const BuildOptions &build,
                      const SystemOptions &sys, std::ostream &err) {
  UniquePtr<IHostMemory> serialized{
...
bool timeRefit(INetworkDefinition const &network,
  ...
  auto const reportMissingWeights = [&] {
    ...
    for (size_t i = 0; i < layerNames.size(); ++i) {
      //TODO
      // FLOWENGINE_LOGGER_ERROR("Missing ({}, {}) for fetting.", layerNames[i], weightsRoles[i]);
  ...
  FLOWENGINE_LOGGER_INFO("Engine refitted in {} ms.", durationMs(refitEndTime - refitStartTime).count() / loop);
  return true;
}
...
void *initSafeRuntime() {
  void *handle{nullptr};
#if !defined(_WIN32)
  std::string const dllName{isDebug()
...
bool checkSafeEngine(void const *serializedEngine, int32_t const engineSize) {
  ...
  if (!hasConsistencyChecker()) {
    FLOWENGINE_LOGGER_ERROR("Cannot perform consistency check because the checker "
                         "is not loaded..");
    return false;
  }
  auto checker = std::unique_ptr<nvinfer1::consistency::IConsistencyChecker>(
      createConsistencyChecker(infer::trt::sample::gLogger.getTRTLogger(), serializedEngine,
                               engineSize));
  if (checker.get() == nullptr) {
    FLOWENGINE_LOGGER_ERROR("Failed to create consistency checker.");
    return false;
  }
  FLOWENGINE_LOGGER_INFO("Start consistency checking.");
  if (!checker->validate()) {
    FLOWENGINE_LOGGER_ERROR("Consistency validation failed.");
    return false;
  }
  FLOWENGINE_LOGGER_INFO("Consistency validation passed.");
  return true;
}
} // namespace sample
} // namespace trt
