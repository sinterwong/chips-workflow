...
PerformanceResult
getPerformanceResult(const std::vector<InferenceTime> &timings,
                     std::function<float(const InferenceTime &)> metricGetter,
                     float percentile) {
  const auto metricComparator = [metricGetter](const InferenceTime &a,
                                               const InferenceTime &b) {
    return metricGetter(a) < metricGetter(b);
  };
  const auto metricAccumulator = [metricGetter](float acc,
                                                const InferenceTime &a) {
...
void printEpilog(const std::vector<InferenceTime> &timings, float walltimeMs,
                 float percentile, int batchSize, std::ostream &osInfo,
  ...
  if (enqueueResult.median >
      enqueueBoundReportingThreshold * gpuComputeResult.median) {
...
void printPerformanceReport(const std::vector<InferenceTrace> &trace,
                            ...
                            int batchSize, std::ostream &osInfo,
                            std::ostream &osWarning, std::ostream &osVerbose) {
  ...
  // parsed through --batch when explicit batch used, batchSize =
  // options.inference.batch = 0 treat inference with explicit batch as a single
  // query and report the throughput
...
//!             "h2d" : time, "compute" : time, "d2h" : time, "latency" : time,
//!             "end to end" : time }
//!
