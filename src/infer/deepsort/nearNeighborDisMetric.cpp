  ...
  for (TRACKER_DATA &data : tid_feats) {
    ...
    if (samples.find(track_id) != samples.end()) { // append
      ...
      if (newSize <= this->budget) {
        FEATURESS newSampleFeatures(newSize, FDIMS);
        newSampleFeatures.block(0, 0, oldSize, FDIMS) = samples[track_id];
        newSampleFeatures.block(oldSize, 0, addSize, FDIMS) = newFeatOne;
        samples[track_id] = newSampleFeatures;
      } else {
        if (oldSize < this->budget) { // original space is not enough;
          FEATURESS newSampleFeatures(this->budget, FDIMS);
          if (addSize >= this->budget) {
            newSampleFeatures = newFeatOne.block(0, 0, this->budget, FDIMS);
          } else {
            newSampleFeatures.block(0, 0, this->budget - addSize, FDIMS) =
                samples[track_id]
                    .block(addSize - 1, 0, this->budget - addSize, FDIMS)
                    .eval();
            newSampleFeatures.block(this->budget - addSize, 0, addSize, FDIMS) =
                newFeatOne;
          }
          samples[track_id] = newSampleFeatures;
        } else { // original space is ok;
          if (addSize >= this->budget) {
            samples[track_id] = newFeatOne.block(0, 0, this->budget, FDIMS);
          } else {
            samples[track_id].block(0, 0, this->budget - addSize, FDIMS) =
                samples[track_id]
                    .block(addSize - 1, 0, this->budget - addSize, FDIMS)
                    .eval();
            samples[track_id].block(this->budget - addSize, 0, addSize, FDIMS) =
                newFeatOne;
          }
        }
      }
...
Eigen::VectorXf
NearNeighborDisMetric::_nneuclidean_distance(const FEATURESS &x,
                                             const FEATURESS &y) {
...
Eigen::MatrixXf
NearNeighborDisMetric::_cosine_distance(const FEATURESS &a, const FEATURESS &b,
