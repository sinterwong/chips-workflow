            ...
            if (c.HasMember("region")) {
              ...
              if (r.IsArray() && r.Size() == 4) {
                for (int i = 0; i < r.Size(); i++) {
                  ...
                  fc.paramsConfig.region[i] = c.GetInt();
                }
              }
            }
            if (c.HasMember("nms_thr")) {
              fc.paramsConfig.nms_thr = c["nms_thr"].GetFloat();
            }
            if (c.HasMember("cond_thr")) {
              fc.paramsConfig.cond_thr = c["cond_thr"].GetFloat();
            }
            if (c.HasMember("modelDir")) {
              fc.paramsConfig.modelDir = c["modelDir"].GetString();
            }
          }
        }
        result.push_back(fc);
